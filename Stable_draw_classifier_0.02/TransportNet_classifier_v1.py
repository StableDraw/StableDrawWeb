...
def main():
    ...
    data_dir = 'C:\\Stable-Draw\\Stable_draw_classifier_0.02\\Dataset_350_v2_transfer'

    image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['train', 'val']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=6, shuffle=True, num_workers=2) for x in ['train', 'val']}
    ...
    def train_model(model, criterion, optimizer, scheduler, num_epochs=100):
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.plot(epoch_loss_list, 'b-', label='train')
        plt.plot(epoch_loss_val_list, 'r-', label='val')
        plt.legend()
        plt.show()
