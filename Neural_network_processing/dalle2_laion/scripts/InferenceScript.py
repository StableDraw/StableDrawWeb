...
class InferenceScript:
    ...
    def _sample_decoder(
        ...
        self.print(f"Семплирование декодера с установленными величинами: {cond_scale}")
            
        decoder_info = self.model_manager.decoder_info
        assert decoder_info is not None, "Декодер не загружен."
        ...
        assert is_valid, f"Требования данных для дектодера не удовлетворены: {errors}"
    ...
    def _sample_prior(self, text_or_tokens: Union[torch.Tensor, List[str]], cond_scale: float = None, sample_count: int = 1, batch_size: int = 100, num_samples_per_batch: int = 2):
        ...
        if cond_scale is None:
            ...
            if cond_scale is None:
                # Fallback
                cond_scale = 1.0
        self.print(f"Первичное семплирование с условным размером: {cond_scale}")
        ...
        assert is_valid, f"Требования к первичным данным не удовлетворены. {errors}"
