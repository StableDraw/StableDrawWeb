...
MODULE_NAME = 'local_fastai'
SOURCE_URL = 'https://github.com/local_fastai/local_fastai/blob/master/'
PYTORCH_DOCS = 'https://pytorch.org/docs/stable/'
local_fastai_DOCS = 'https://docs.fast.ai'
use_relative_links = True

_typing_names = {t:n for t,n in local_fastai_types.items() if t.__module__=='typing'}
...
def link_type(arg_type, arg_name=None, include_bt:bool=True):
    ...
    if is_local_fastai_class(arg_type): return f'[{arg_name}]({get_fn_link(arg_type)})'
    return arg_name

def is_local_fastai_class(t): return belongs_to_module(t, MODULE_NAME)
...
def format_ft_def(func, full_name:str=None)->str:
    ...
    if is_local_fastai_class(type(func)):        arg_str += f" :: {link_type(type(func))}"
...
def show_doc(elt, doc_string:bool=True, full_name:str=None, arg_comments:dict=None, title_level=None, alt_doc_string:str='',
    ...
    source_link = get_function_source(elt) if is_local_fastai_class(elt) else ""
...
def find_elt(modvars, keyword, match_last=False):
    ...
    keyword = strip_local_fastai(keyword)
...
def get_fn_link(ft)->str:
    ...
    anchor = strip_local_fastai(get_anchor(ft))
    module_name = strip_local_fastai(get_module_name(ft))
    base = '' if use_relative_links else local_fastai_DOCS
