...
class TestRegistry:
    ...
    def this_tests(*funcs):
        ...
        for func in funcs:
            ...
            if re.match(r'local_fastai\.', func_fq):
                if entry not in TestRegistry.registry[func_fq]:
                    TestRegistry.registry[func_fq].append(entry)
            else:
                raise Exception(f"'{func}' is not in the local_fastai API") from None
...
def str2func(name):
    "Converts 'local_fastai.foo.bar' into an function 'object' if such exists"
...
def relative_test_path(test_file:Path)->str:
    "Path relative to the `local_fastai` parent directory"
