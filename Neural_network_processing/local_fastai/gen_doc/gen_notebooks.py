...
def create_module_page(mod, dest_path, force=False):
    ...
    strip_name = strip_local_fastai(mod_name)
    ...
    cells = [get_code_cell(f'from local_fastai.gen_doc.nbdoc import *\nfrom {mod_name} import * ', True)]
...
def get_doc_path(mod, dest_path):
    strip_name = strip_local_fastai(mod.__name__)
...
def update_nb_metadata(nb_path=None, title=None, summary=None, keywords='local_fastai', overwrite=True, **kwargs):
...
IMPORT_RE = re.compile(r"from (local_fastai[\.\w_]*)")
...
def get_top_level_modules(num_levels=1):
    mod_dir = Path(import_mod('local_fastai').__file__).parent
...
def get_module_from_notebook(doc_path):
    "Find module given a source path. Assume it belongs to local_fastai directory"
    return f'local_fastai.{Path(doc_path).stem}'
...
def update_notebooks(source_path, dest_path=None, update_html=True, document_new_fns=False,
    ...
    "`source_path` can be a directory or a file. Assume all modules reside in the local_fastai directory."
    ...
    if source_path.is_file():
    ...
    elif (source_path.name.startswith('local_fastai.')):
        # Do module update
        ...
        doc_path = Path(dest_path)/(strip_local_fastai(mod.__name__)+'.ipynb')
