...
from local_fastai.gen_doc import nbdoc
...
def show_test(elt)->str:
    "Show associated tests for a local_fastai function/class"
...
def find_related_tests(elt)->Tuple[List[Dict],List[Dict]]:
    "Searches `local_fastai/tests` folder for any test functions related to `elt`"
    related_matches = []
...
def get_tests_dir(elt)->Path:
    "Absolute path of `local_fastai/tests` directory"
...
def get_file(elt)->str:
    ...
    if not nbdoc.is_local_fastai_class(elt): return None
    return inspect.getfile(elt)

def find_test_files(elt, exact_match:bool=False)->List[Path]:
    "Searches in `local_fastai/tests` directory for module tests"
