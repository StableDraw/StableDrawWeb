...
# python -c 'import local_fastai.datasets; print(local_fastai.datasets._check_file("mnist_var_size_tiny.tgz"))'
_checks = {
...
class Config():
    "Creates a default config file 'config.yml' in $local_fastai_HOME (default `~/.local_fastai/`)"
    DEFAULT_CONFIG_LOCATION = os.path.expanduser(os.getenv('local_fastai_HOME', '~/.local_fastai'))
    ...
    @classmethod
    def model_path(cls):
        "Get the path to local_fastai pretrained models in the config file."
