...
def Delete_background(binary_data):
    test_salobj_dataset = SalObjDataset(binary_data = binary_data, lbl_name_list = [], transform = transforms.Compose([RescaleT(320), ToTensorLab(flag = 0)])) #Загрузчик даных
    ...
    for data in test_salobj_dataloader: #вывод для каждого изображения
        ...
        orig = Image.open(io.BytesIO(binary_data)).convert("RGB")
