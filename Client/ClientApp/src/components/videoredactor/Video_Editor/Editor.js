...
class Editor extends React.Component {
    ...
    componentDidMount = () => {
        ...
        this.playVideo.current.addEventListener('timeupdate', function () {
            ...
            let seek = (self.playVideo.current.currentTime - self.props.timings[curr_idx].start) / self.playVideo.current.duration * 100;
    ...
    skipPrevious = () => {
        ...
        let prev_idx = (this.state.currently_grabbed.index !== 0) ? (this.state.currently_grabbed.index-1) : (this.props.timings.length-1)
    ...
    skipNext = () => {
        ...
        let next_idx = (this.state.currently_grabbed.index !== (this.props.timings.length-1)) ? (this.state.currently_grabbed.index+1) : 0
    ...
    updateProgress = (event) => {
        ...
        let seekTime = ((event.clientX - playbackRect.left) / playbackRect.width) * this.playVideo.current.duration
        this.playVideo.current.pause()
                ...
                <div className="wrapper">
                    ...
                    <div className="controls">
                        <div className="player-controls">
                            <button className="settings-control" title="Перезапустить видео" onClick={this.reset}><FontAwesomeIcon icon={faSync} /></button>
                            <button className="settings-control" title="Убрать/Включить звук" onClick={() => this.setState({isMuted: !this.state.isMuted})}>{this.state.isMuted ? <FontAwesomeIcon icon={faVolumeMute} /> : <FontAwesomeIcon icon={faVolumeUp} />}</button>
                            <button className="settings-control" title="Сделать снимок кадра" onClick={this.captureSnapshot}><FontAwesomeIcon icon={faCamera} /></button>
                        </div>
                        <div className="player-controls">
                            <button className="seek-start" title="Прошлая точка" onClick={this.skipPrevious}><FontAwesomeIcon icon={faStepBackward} /></button>
                            <button className="play-control" title="Воспроизвести видео/ Пауза" onClick={this.play_pause.bind(this)} >{this.state.playing ? <FontAwesomeIcon icon={faPause} /> : <FontAwesomeIcon icon={faPlay} /> }</button>
                            <button className="seek-end" title="Следующая точка" onClick={this.skipNext}><FontAwesomeIcon icon={faStepForward} /></button>
                        </div>
                        <div>
                            <button title="Add grabber" className="trim-control margined" onClick={this.addGrabber}>Добавить точку <FontAwesomeIcon icon={faGripLinesVertical} /></button>
                            <button title="Delete grabber" className="trim-control margined" onClick={this.preDeleteGrabber}>Удалить точку <FontAwesomeIcon icon={faGripLinesVertical} /></button>
                            <button title="Save changes" className="trim-control" onClick={this.saveVideo}>Сохранить</button>
                        </div>
                    </div>
                    ...
                    {(this.state.imageUrl !== "") ? 
                        <div className={"marginVertical"}>
                            ...
                            <div className="controls">
                                <div className="player-controls">
                                    <button className="settings-control" title="Скачать кадр" onClick={this.downloadSnapshot}><FontAwesomeIcon icon={faDownload} /></button>
                                    <button className="settings-control" title="Удалить кадр" onClick={() => {this.setState({imageUrl: ""})}}><FontAwesomeIcon icon={faEraser} /></button>
                                </div>
                            </div>
                        </div>
                    : ""}
